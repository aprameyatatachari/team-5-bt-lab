...
public interface CustomerProofOfIdentityRepository extends JpaRepository<CustomerProofOfIdentity, String> {
    ...
    List<CustomerProofOfIdentity> findByCustomerCustomerNumber(String customerNumber);
    
    List<CustomerProofOfIdentity> findByProofOfIdType(String proofOfIdType);
    
    Optional<CustomerProofOfIdentity> findByCustomerCustomerNumberAndProofOfIdType(String customerNumber, String proofOfIdType);
    
    @Query("SELECT c FROM CustomerProofOfIdentity c WHERE c.customer.customerNumber = :customerNumber ORDER BY c.effectiveDate DESC")
    ...
    @Query("SELECT c FROM CustomerProofOfIdentity c WHERE c.customer.customerNumber = :customerNumber AND (c.endDate IS NULL OR c.endDate >= :currentDate)")
